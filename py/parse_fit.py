...
POINTS_COLUMN_NAMES = ['timestamp', 'lat', 'lon', 'lap', 'altitude', 'heart_rate', 'cadence', 'speed', 'distance', 'enhanced_speed']
...
def get_fit_point_data(frame: fitdecode.records.FitDataMessage) -> Optional[Dict[str, Union[float, int, str, datetime]]]:
    ...
    data['timestamp'] = frame.get_value('timestamp')  # This is a datetime object
    ...
    for field in POINTS_COLUMN_NAMES[4:]:
        if frame.has_field(field):
            data[field] = frame.get_value(field)
...
def list_fields(fname:str):
    """Print the names of all fields in a FIT frame.
    """
    first_frame = True
    with fitdecode.FitReader(fname) as fit_file:
        for frame in fit_file:
            if isinstance(frame, fitdecode.records.FitDataMessage):
                if frame.name == 'record' and first_frame:
                    first_frame = False
...
def get_dataframes(fname: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    points_df.set_index('timestamp', inplace=True)
